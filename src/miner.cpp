...
void static BitcoinMiner(CWallet *pwallet, int blockCount)
{
	...
	unsigned int nBlocksGenerated = 0;
	...
	//            MilliSleep(1000);
			...
			*/
			...
			while (true)
			{            
				if (blockCount > 0 && nBlocksGenerated >= blockCount)
				{
					LogPrintf("Generated all required blocks: %d >= %d\n", nBlocksGenerated, blockCount);
					return;
				}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads, int blockCount)
{
	...
	minerThreads->create_thread(boost::bind(&BitcoinMiner, pwallet, blockCount));
}
#endif
